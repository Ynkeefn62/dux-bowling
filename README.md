...
- `Dux Stripe` (outline) — only the neon BOWLING wordmark on the homepage.
- `Dux Stripe Solid` — headings, eyebrows, buttons, nav, stats. All caps by design.
- `Nunito` (SIL OFL, self-hosted in `public/fonts`, licence alongside) — all reading
  copy: paragraphs, tiles, form labels, inputs, legal pages. Proper case with real
  lowercase, so body text never reads as shouting.

The two Dux fonts had their `4` and `A` counters repaired on 2026-09-08.
