  ...
  async function submit() {
    ...
    try {
      const res = await fetch('/api/bowler-signup', {
        ...
        body: JSON.stringify({ name, email, home_alley: alley, show_on_board: show }),
      });
      const j = await res.json();
      ...
      <div className="card" style={{ borderColor: '#6b3a1e' }}>
        {done ? (
          <div style={{ textAlign: 'center', padding: '18px 0' }}>
            <h3 className="stripe sub">You&apos;re on the list.</h3>
            <p className="lede" style={{ margin: '8px auto 0' }}>
              We&apos;ll let you know the moment there is a Dux lane you can bowl on.
            </p>
          </div>
        ) : (
          <>
