...
/**
 * Defined at module scope on purpose. A component declared inside AlleyForm
 * would be a brand-new component type on every keystroke, so React would
 * unmount and remount the textarea, dropping focus and closing the mobile
...
function TextQuestion({
  ...
  return (
    <div className="field">
      ...
      <textarea id={id} className="ta" value={value} onChange={(e) => onChange(id, e.target.value)} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        {SHERMAN_Qs.map((q) => (
          <TextQuestion key={q.id} {...q} value={s(q.id)} onChange={set} />
        ))}
      </div>
