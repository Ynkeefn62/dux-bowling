  ...
  return (
    <div className="exhibit">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label={`${ex.label} positioning chart`}>
        <rect x={px(0.5)} y={py(1)} width={px(1) - px(0.5)} height={py(0.5) - py(1)} fill="var(--panel-2)" opacity="0.9" />
        <line x1={px(0.5)} y1={py(0)} x2={px(0.5)} y2={py(1)} stroke="var(--line)" strokeDasharray="2 4" />
        <line x1={px(0)} y1={py(0.5)} x2={px(1)} y2={py(0.5)} stroke="var(--line)" strokeDasharray="2 4" />
        <line x1={px(0)} y1={py(0)} x2={px(1) + 12} y2={py(0)} stroke="var(--ghost)" strokeWidth="1.3" />
        <line x1={px(0)} y1={py(0)} x2={px(0)} y2={py(1) - 12} stroke="var(--ghost)" strokeWidth="1.3" />
        <text x={px(0)} y={py(0) + 22} fill="var(--dim)" fontSize="9.5" letterSpacing="1.4">{ex.xLeft.toUpperCase()}</text>
        <text x={px(1) + 12} y={py(0) + 22} fill="var(--dim)" fontSize="9.5" letterSpacing="1.4" textAnchor="end">{ex.xRight.toUpperCase()}</text>
        <text x={(px(0) + px(1)) / 2} y={py(0) + 42} fill="var(--ink)" fontSize="11" letterSpacing="2" textAnchor="middle">{ex.xTitle.toUpperCase()}</text>
        <text transform={`translate(${px(0) - 16} ${py(0)}) rotate(-90)`} fill="var(--dim)" fontSize="9.5" letterSpacing="1.4">{ex.yLow.toUpperCase()}</text>
        <text transform={`translate(${px(0) - 16} ${py(1) - 12}) rotate(-90)`} fill="var(--dim)" fontSize="9.5" letterSpacing="1.4" textAnchor="end">{ex.yHigh.toUpperCase()}</text>
        <text transform={`translate(${px(0) - 40} ${(py(0) + py(1)) / 2}) rotate(-90)`} fill="var(--ink)" fontSize="11" letterSpacing="2" textAnchor="middle">{ex.yTitle.toUpperCase()}</text>

        {ex.points.map((p) => {
          const on = p.id === active.id;
          const fill = p.hero ? 'var(--orange)' : p.ghost ? 'var(--ghost)' : 'var(--ink)';
          return (
            <g key={p.id} className="dot-hit" onClick={() => setSel(p.id)}>
      ...
      <p style={{ color: 'var(--ghost)', fontSize: 11.5, lineHeight: 1.65, marginTop: 10 }}>{ex.note}</p>
    </div>
  );
}
