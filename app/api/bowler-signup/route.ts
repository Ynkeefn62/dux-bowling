...
export async function POST(req: Request) {
  ...
  const frequency = String(body.frequency ?? '').trim().slice(0, 60);
  const leagues = String(body.leagues ?? '').trim().slice(0, 60);
  const tournaments = String(body.tournaments ?? '').trim().slice(0, 60);
